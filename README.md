...
**Darbo su .dmg failu instrukcija:** repozitorijoje yra prisegtas failas "galutine.dmg", skirtas atlikti panašią funkciją, kurią atlieka setup.exe/msi failai. Jų nebuvo galima realizuoti, kadangi dirbama ant macOS opearicne sistema. Todėl buvo sukurtas .dmg failas. Tai atminties diskas, kuris savyje laiko šios programos .exe failą bei pavyzdinį tektstinį failą, jeigu būtų norima išsitestuoti darbą su failais. Su .dmg failo pagalba, galima dalintis .exe failu su kitais macOS naudotojais. _Kaip jis veikia:_ du kartus paspaudus failą (geriausia jį įsikelti į darbalaukį), darbalaukyje atsiranda diskas, kuriame laikomi minėti .exe ir .txt failai. Atidarius diską, atsiranda langas su failais, kuriuos vartotojas jau gali pasiekti ir išsikelti į kokią nori direktoriją (tarkim darbalaukį). Tuomet vartotojas gali paleisti sukurtą programą (.exe failą). Norint panaikinti atsiradusį diską, reikia ant jo paspausti dešinį klavišą ir pasapusti "eject".

**Papildoma informacija**: taip pat prie visų failų yra prisegtas "DMG.sh" failas. Jį paleidus per terminalą ir turint .dmg failą darbalaukyje, .exe ir .txt failai atsiranda darbalaukyje ir papildomoje direktorijoje (šiuo metu nustatyta, kad keliautų į VU/Vardenis-Pavrdenis direktoriją). T. y. šis .sh failas automatiškai atidaro tiek .dmg failą, tiek diską, sukelia failus, kur jiems nurodyta nukeliauti, ir tada uždaro diską. Tačiau norint, kad šis failas veiktų reikia atlikti keletą veiksmų:
1. Atidarius DMG.sh failą, reikės pakeisti pirmų trijų eilučių direktorijas (pirma skirta pasiekti .dmg failą, antra vienai direktorijai, trečia darbalaukiui).
